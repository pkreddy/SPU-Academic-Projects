...
#convergence limit calculated in this abs(newlikelihood / LogsumLoglikelihood_old)
...
#reading ydata.txt 
data_file <- read.csv('E:\\Google Drive\\Fall 17 Sem 2\\630 Machine Learning\\Project-1\\cluster.csv',header = FALSE,numerals = "no.loss")
...
#LogsumLoglikelihood_old <- log(sumLoglikelihood) * dim(newdf)[1]
...
repeat{
  ...
  #}
  #test
  #test <- dmvnorm(as.matrix(newdf[1,2:71]),mean = as.matrix(get(paste("new_mean",1,sep = ""))),sigma = diag(get(paste("new_covariance",1,sep = ""))))
  #newLogsumLoglikelihood_old <- log(newsumLoglikelihood) * dim(newdf)[1]
  ...
  print(paste("absabs((newlikelihood - LogsumLoglikelihood_old )/LogsumLoglikelihood_old) * 100 =",
